...
Trace mode is a live-state debug overlay on the status page. Activated by the `[Trace: OFF/ON]` toggle; the toggle is disabled until `[▶ Test]` has been pressed at least once for this piston.

**Mechanism: live-tracking while the status page is open**

While trace is active, PistonCore subscribes to changes for only this piston's entities — the bounded list named by its own statement nodes. Use the WebSocket `subscribe_entities` command with an explicit `entity_ids` list (returns a keyed entity map and compact diffs) rather than a system-wide `subscribe_events` / `state_changed` stream. This avoids noise and scales with the piston's device count. Each change delivers `old_state` → `new_state` plus the event's `context`.

PistonCore evaluates each condition node against that live state by reading the node's intent (entity, operator, operand) from the **stable PistonCore JSON** — never from the compiled output. Colors cannot drift when the compiled form shifts. Tracking is live-only: it sees changes while the page is open; it does not replay past runs.

**Condition coloring**

- **State conditions** (`is`, `is not`, `is less than`, `is between`, etc.) — compare against current entity state. Color reliably whenever the page is open.
- **Transition conditions** (`changes to`, `changes away from`) — true only at the instant of a transition. Colorable only when PistonCore witnesses the change live while the page is open; a transition that settled before the page opened has no event to replay.
...
The receipt serves two jobs: proof an action fired when you can't see or hear it (a silent notify, a variable set, a scene in another room), and proof *this piston* caused it — not another automation or a coincidence. Correlation by timing alone cannot prove causation; context attribution closes this.

How it works:
- **Capture run context:** when PistonCore fires the piston, capture the run's HA `context.id` from the `script_started` / `call_service` event the run produces.
- **Start mark:** the instant PistonCore's own evaluation flips the last blocking condition of a branch to true ("all conditions met").
- **Stop mark:** the first effect event whose `context` traces to this run's `context.id` (directly or via `parent_id`) — not merely the first change on the target entity.
- **Correlation window:** the effect must land within a bounded window of the start mark. Default **5–15 seconds** (sized for typical device latency). **Make this configurable in settings.** Context narrows *who* caused it; the window narrows *when* — both guards together reject stale or coincidental same-context matches.
- **Receipt:** the delta is shown as a measured wall-clock duration.

**Three receipt states — required, must be honest:**
- **Verified** — effect landed, context traces to this run, within window → proof the piston caused it.
- **None** — conditions met, no matching effect within window → the action did not fire, or fired and did nothing. The diagnostic tell.
- **Unverified** — an effect was seen but context could not be tied to this run → shown as *"changed, but we can't confirm we caused it."* Never silently shown as success.

**Effect / receipt source by action type:**
- Visible device action (turn on/off, set level) → target entity's `state_changed`, context-matched
- Variable set → the variable's own state update, context-matched
- Notify / speak with no visible target → the `call_service` event for that action, context-matched

Every action type needs a defined, context-matchable proof signal. Receipt Sources Reference table is an open testbench item — see open items below.

**Context reliability — why Unverified is a real, expected state:**
Some integrations or device-driven changes arrive with `parent_id: null` or absent context propagation. Highly parallel or async branches can muddy `parent_id`. PistonCore falls back to **Unverified** with clear UI labeling in any such case — expected behavior, not a bug.

**Honest labeling (required):** Duration is end-to-end wall-clock — includes HA latency, network, and device response. Label it as such (e.g. *"condition met → device responded: 340ms"*). Never present it as "how long the piston took." A slow bulb must not read as a slow piston.

**PyScript upgrade path**

On a PyScript target, PistonCore controls the runtime, so the native-YAML ceiling lifts:
- Full per-node timing — runtime records offset + duration for every node, including false conditions (true WebCoRE `trace.points` parity)
- Custom per-node events — runtime emits condition pass/fail directly instead of inferring from live state
- Historical replay — feasible if PistonCore stores each run; PyScript gives the control to record; run storage is a feature to build, not free

**What is permanently gone on native YAML**
- Per-node execution timing (WebCoRE's `+12ms 2ms` per line) — engine-internal; live-tracking has no access and must not fake it. Available on PyScript.
- Replay of a run that already settled — live-tracking only sees changes while the page is open. Available on PyScript with run storage.

**Required bottom-of-screen disclaimer**

A persistent notice at the bottom of the status/debug screen. **Evergreen wording — states only the permanent architectural fact, no implementation detail, so it never goes stale:**

> *PistonCore's trace is not a complete clone of WebCoRE's. WebCoRE's hub engine reported the evaluated result and timing of every node during execution. PistonCore on native HA YAML has no access to engine-internal evaluation.*

Full user-facing description for docs / help / hover (not the on-screen line — this names implementation that may change):

> *PistonCore's trace is a practical reconstruction, not a full WebCoRE clone. WebCoRE's engine reported every node evaluation and timing. PistonCore on native HA YAML uses live state changes + context attribution for reliable proof of causation where possible. Full per-node detail and replay are available on PyScript pistons.*

**Open items — confirm at testbench (priority order)**

1. Receipt-source map per action type — context-matchable proof signal for every action type; build the Receipt Sources Reference table (add to `reference/TRACE_SPEC_SECTION.md`).
2. Aggregation evaluation (`Any/All/None of {group}`) logic; v1 = Any/All/None only.
3. `subscribe_entities` performance with large entity lists — confirm stays responsive for the biggest real pistons.
4. Context capture + match reliability on native YAML — confirm capture of run `context.id` at fire time; catalog which action types fall back to Unverified.
5. Correlation window default — confirm 5–15s default and the settings control.
6. Operator coverage for the evaluator (`is`, `is not`, `<`, `between`, `changes to`, `changes away from`, `any/all/none of`); unsupported → neutral.
7. Start-mark attribution — tie "last blocking condition met" to the correct branch when multiple branches share entities.
