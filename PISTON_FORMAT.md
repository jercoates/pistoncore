...
The compiler reads structured JSON only. It never reads piston_text or Snapshot JSON.
The wizard writes structured JSON only. It never writes display text.
...
| `piston_text` | string | No | Human-readable render of the piston. Generated by frontend render functions on save. Never parsed. Never authoritative. JSON is always the source of truth. **TEMPORARY** — remove after S3-1 testing confirms editor renders reliably. |
...
> This field is generated from the same render functions the editor uses — not a
> parallel render path. If the editor fails to render any statement, `piston_text`
> is **not** regenerated — the previous value is preserved. A piston where the editor
> renders 17 of 18 statements must not produce a `piston_text` showing only 17, as
> that would make the user think a statement was deleted.
> **This field will be removed after S3-1 round-trip testing confirms editor
> render-back is reliable for all 18 statement types.**
...
- **Not piston_text** — `piston_text` is a temporary human-readable safety net stored at the bottom of the piston JSON during active editor development. It is never parsed by the compiler, backend, or import flow. JSON is always the source of truth. See the warning block in the Wrapper Field Reference above.
