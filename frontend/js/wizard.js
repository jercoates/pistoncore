...
//
// Session 43 (W-S5) — Wizard fixes
//   _condId(): new helper — condition IDs use cond_ prefix (not stmt_)
//   _buildConditionNode(): uses _condId() instead of _newId()
//   _route() edit-condition: handles flat-field format (imported pistons have role/attribute
//     directly on condition, no subject object) — pre-fills wizard correctly for edit
    ...
    if (_editNode) {
      ...
      if (t === 'trigger' || t === 'condition' || t === 'restriction' || ctx === 'edit_condition') {
        ...
        if (_editNode.subject) {
          // Wizard-written format: subject object present
