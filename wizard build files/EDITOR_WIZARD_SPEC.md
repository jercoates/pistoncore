...
**Version:** 2.2 (added §5.1 vocab-driven do-list, §7.6 custom command params, §8.7 offline device reconciliation, §9.5 extended upgrade paths, §10 cross-dialog behaviors; renumbered §10–§14 → §11–§15; EDITOR-GAP-6 filed)
...
**Restriction upgrade (`upgradeRestriction`).** The function exists in WebCoRE source and the button is coded with label "Convert to new group," but it is permanently hidden in WebCoRE by `ng-if="(0==1)"` — never surfaced in the real WebCoRE UI. Whether PistonCore surfaces this path is an open design choice.
...
`[DECISION PENDING: expose upgradeRestriction() in PistonCore or leave hidden — do not implement until Jeremy decides]`
