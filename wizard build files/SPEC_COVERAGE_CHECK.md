...
## ⚠ PROVENANCE — read before trusting any behavior claim

This file mixes two kinds of content. They are NOT equally reliable. If a build goes wrong, the inferred items are where to look first.

- **`[EXTRACTED]` — fact, read from source.** That a button/handler EXISTS and what it CALLS comes directly from `piston.module.html.txt` (the `ng-click` strings). Solid. The gap diff itself (button exists in source, spec omits it) is extracted and trustworthy.
- **`[INFERRED]` — reasoning, NOT read from source.** What each handler DOES internally (commits, resets, advances, declares, wraps, pastes) was reasoned from the WebCore pattern and general WebCore knowledge. **The JS that defines these handlers (`piston.module.js`) was never available.** The provided `app.js` is the dashboard/dataService layer and contains none of these editor handlers — confirmed by search. So every "what it does" description below is an educated inference, not an extraction. Treat as a lead to verify, not as fact.

The handoff already routes this correctly: extracted facts → `[VERIFIED]` tags in the spec; inferred behavior → `[DECISION: derived from WebCore behavior]`. Nothing inferred is allowed into the spec as `[VERIFIED]`.
...
**What it is:** `[INFERRED — not read from JS]` a footer button that commits the current item, keeps the dialog open, resets the scratch buffer, and advances the insert cursor — so the user adds several of the same thing without the dialog closing. `[EXTRACTED]` The button exists and is labeled "Add more", calling `updateX(true)`, on 5 dialogs (event, condition, task, variable, global-variable) — this part is read from the HTML. The internal commit/reset/advance behavior is inferred.
...
**Spec:** absent.
**What it is:** `[EXTRACTED]` the operand shows an error with a "(create it)" link calling `autoAddVariable()` — confirmed in HTML, 20 sites. `[INFERRED — not read from JS]` that invoking it declares the variable on the spot and re-validates without leaving the dialog — reasoned, not extracted.
...
**What it is:** `[EXTRACTED]` a "From clipboard" section with `pasteItem`/`deleteClipboardItem` appears in 4 add-dialogs — confirmed in HTML. `[INFERRED — not read from JS]` that it pastes the held node into the current target array at the insert position — reasoned.
...
**Spec:** absent.
**What it is:** `[EXTRACTED]` `addParameter(type)`/`deleteParameter` exist, shown when `designer.custom` — confirmed in HTML. `[INFERRED — not read from JS]` the exact builder flow.
...
**What it is:** `[EXTRACTED]` `upgradeStatement`/`upgradeRestriction`/`upgradeCondition` buttons exist labeled "Convert to new group" — confirmed in HTML. `[INFERRED — not read from JS]` the wrap-into-group mechanics (the condition case IS documented in spec §9.5; statement/restriction extrapolated from it).
